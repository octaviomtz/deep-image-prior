...
path_img_dest = '/data/OMM/project results/Feb 5 19 - Deep image prior/dip results all 17/'
...
for idx_name, name in enumerate(ids_missing):
    ...
    for (block, block_mask, block_maxvol_and_lungs, block_lungs, block_name) in zip(blocks_ndl, blocks_ndl_mask, block_mask_maxvol_and_lungs, blocks_ndl_lungs, blocks_ndl_names): 
        ...
        # LR FOUND
        try:
            LR = np.load(f'{path_img_dest}learning rates/v17v2_merged_clusters/{name}_{block_name}.npy')
        ...
        stop = time.time()
        np.save(f'{path_img_dest}v17v2_merged_clusters/arrays/last/{name}_{block_name}.npy',image_last)
        np.save(f'{path_img_dest}v17v2_merged_clusters/arrays/orig/{name}_{block_name}.npy',img_np)
        np.savez_compressed(f'{path_img_dest}v17v2_merged_clusters/arrays/masks/{name}_{block_name}',block_maxvol_and_lungs)
        np.savez_compressed(f'{path_img_dest}v17v2_merged_clusters/arrays/masks nodules/{name}_{block_name}',block_mask)
        np.savez_compressed(f'{path_img_dest}v17v2_merged_clusters/arrays/masks lungs/{name}_{block_name}',block_lungs)
        np.save(f'{path_img_dest}v17v2_merged_clusters/mse error curves inpainting/{name}_{block_name}.npy',mse_error)
        np.save(f'{path_img_dest}v17v2_merged_clusters/inpainting times/{name}_{block_name}.npy',stop-start)
